...
class Entities extends EventEmitter {
  ...
  constructor(id) {
    ...
    this.id = id;
    this.#storage = {};
    this.#dataPath = process.env.UC_CONFIG_HOME || "";
    this.#configFileName = path.join(this.#dataPath, `${this.id}_entities.json`);

    // load configured entities
    if (fs.existsSync(this.#configFileName)) {
      const raw = fs.readFileSync(this.#configFileName);

      try {
        this.#storage = JSON.parse(raw);
        console.log(`ENTITIES(${this.id}): config file loaded.`);
      } catch (e) {
        console.log(`ENTITIES(${this.id}): Error parsing entities: ${this.#configFileName}`);
      }
    } else {
      console.log(`ENTITIES(${this.id}): No storage file, will create one later: ${this.#configFileName}`);
    }
  }
  ...
  saveData() {
    try {
      fs.writeFileSync(this.#configFileName, JSON.stringify(this.#storage));
      console.log(`ENTITIES(${this.id}): Config saved to file: ${this.#configFileName}`);
    } catch (e) {
      console.error(`ENTITIES(${this.id}): Error writing config: ${this.#configFileName}`);
    }
  }
}
