...
test("createUiText with an EntityCommand creates a command with params", (t) => {
    const result = createUiText("test", 2, 3, new EntityCommand("my_command", { param1: "test", param2: 42, param3: ["foo", "bar"] }));
    t.true(result instanceof UiItem, "result must be a UiItem");
    t.deepEqual(JSON.parse(JSON.stringify(result)), {
        type: "text",
        location: { x: 2, y: 3 },
...
test("createUiIcon with an EntityCommand creates a command with params", (t) => {
    const result = createUiIcon("uc:star", 2, 3, new EntityCommand("my_command", { param1: "test", param2: 42, param3: ["foo", "bar"] }));
    t.true(result instanceof UiItem, "result must be a UiItem");
    t.deepEqual(JSON.parse(JSON.stringify(result)), {
        type: "icon",
        location: { x: 2, y: 3 },
