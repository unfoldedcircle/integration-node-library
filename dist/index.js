...
class IntegrationAPI extends EventEmitter {
    ...
    init(driverConfig, setupHandler) {
        ...
        log.info("Driver is up: %s, version: %s, listening on: %s:%d", this.driverInfo.driver_id, this.driverInfo.version, integrationInterface || "0.0.0.0", port);
    }
    getConfigDirPath() {
    ...
    #authentication(wsId, success) {
        this.#sendResponse(wsId, 0, uc.MESSAGES.AUTHENTICATION, {}, success ? uc.STATUS_CODES.OK : uc.STATUS_CODES.UNAUTHORIZED);
    }
    #getDeviceState() {
        return {
            state: this.state
    ...
    async #entityCommand(wsId, reqId, data) {
        ...
        if (!entity.hasCmdHandler) {
            ...
            log.warn(`DEPRECATED no entity command handler provided for ${data.entity_id} by the driver: please migrate the integration driver, the legacy ENTITY_COMMAND event will be removed in a future release!`);
            ...
            const resultEnumValue = STATUS_CODES[result];
    ...
    async #setupDriver(wsId, reqId, data) {
        ...
        // legacy: emit event, so the driver can act on it
        if (!this.setupHandler) {
            log.warn("DEPRECATED no setup handler provided by the driver: please migrate the integration driver, the legacy SETUP_DRIVER, SETUP_DRIVER_USER_DATA, SETUP_DRIVER_USER_CONFIRMATION events will be removed in a future release!");
        ...
        try {
            ...
            if (action instanceof uc.setup.RequestUserInput) {
                ...
                await this.driverSetupProgress(wsHandle);
                await this.requestDriverSetupUserConfirmation(wsHandle, action.title, String(action.header), undefined, String(action.footer));
                result = true;
            }
    ...
    async setDeviceState(state) {
        this.state = state;
        await this.#broadcastEvent(uc.MSG_EVENTS.DEVICE_STATE, {
            state: this.state
     ...
     */
    async requestDriverSetupUserConfirmation(wsHandle, title, msg1, image = undefined, msg2) {
        const msgData = {
            event_type: "SETUP",
